...
{
	GAMEPLAYATTRIBUTE_REPNOTIFY(ULevelAttributeSet, Level, OldValue, REPNOTIFY_Always);
...
{
	GAMEPLAYATTRIBUTE_REPNOTIFY(ULevelAttributeSet, XP, OldValue, REPNOTIFY_Always);
...
{
	GAMEPLAYATTRIBUTE_REPNOTIFY(ULevelAttributeSet, XPToNextLevel, OldValue, REPNOTIFY_Always);
...
void ULevelAttributeSet::PostGameplayEffectExecute(const FGameplayEffectModCallbackData& Data)
{
	...
	{
		SetXP(FMath::Clamp(GetXP(), 0.0f, GetXPToNextLevel()));
	}
	if (Data.EvaluatedData.Attribute == GetXPGainAttribute())
	{
		
	}
	if (Data.EvaluatedData.Attribute == GetXPToNextLevelAttribute())
	{
